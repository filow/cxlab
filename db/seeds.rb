...
def update_record(key,val,sort,pid=0)
    ...
    # 获取当前数据库中的对应记录
    db_item=Manage::Config.find_by_name(key)
    ...
    if db_item
        ...
        # 记录不存在，创建记录
        db_item=Manage::Config.create val.merge({"name"=>key}).merge(extra_flag)
        puts "\033[36m[添加]\033[0m  记录#{db_item.name}:#{db_item.title}"
    end
    return db_item
end
...
configs.each do |key,val|
    ...
    # 更新父节点
    parent_config=update_record(key,val,parent_sort)
    parent_sort+=1
    ...
    if children
        ...
        children.each do |child_k,child_v|
            # 子节点会把父节点的名字当做前缀
            update_record(parent_config.name + '_' + child_k,child_v,child_sort,parent_node.id)
            child_sort+=1
        end
    end
end
